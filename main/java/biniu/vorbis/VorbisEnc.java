...
public class VorbisEnc {
    ...
    static final InfoMode[] _mode_template = {
    ...
    private void setTonemask(double s, int block, Att3[] att,
        ...
        int i, is = (int) s;
        // clamp 避免越界 (is 和 is+1 都要合法，is 最大 = maxLen-2)
        int maxLen = Math.min(att.length, Math.min(max.length, in.length));
        if (is < 0) is = 0;
        if (is > maxLen - 2) is = maxLen - 2;
        double ds = s - is;
        PsyInfo p = ci.psyParam[block];
        ...
        // 循环上界取 P_BANDS 与 block 实际长度的较小值，避免越界
        int bands = Math.min(Psy.P_BANDS,
                Math.min(in[is].block.length, in[is + 1].block.length));
        for (i = 0; i < bands; i++)
    ...
    private void setCompandBlock(double s, int block, CompandBlock[] in, double[] x) {
        int i, is = (int) s;
        // clamp 避免越界 (is 和 is+1 都要合法)
        if (is < 0) is = 0;
        if (is > x.length - 2) is = x.length - 2;
        double ds = s - is;
        ...
        // 二次 clamp (x 映射后)
        if (is < 0) is = 0;
        if (is > in.length - 2) is = in.length - 2;
    ...
    private void setPeak(double s, int block, int[] suppress) {
        ...
        // clamp 避免越界 (is 和 is+1 都要合法)
        if (is < 0) is = 0;
        if (is > suppress.length - 2) is = suppress.length - 2;
        double ds = s - is;
        PsyInfo p = ci.psyParam[block];
    ...
    private void setNoisebias(double s, int block, int[] suppress,
        ...
        int i, is = (int) s, j;
        // clamp 避免越界；取所有相关数组的最短长度
        int maxLen = Math.min(suppress.length, in.length);
        if (is < 0) is = 0;
        if (is > maxLen - 2) is = maxLen - 2;
        double ds = s - is;
        PsyInfo p = ci.psyParam[block];
        ...
        // 循环上界取 P_* 与实际数据长度的较小值，避免越界
        int curves = Math.min(Psy.P_NOISECURVES, in[is].data.length);
        for (j = 0; j < curves; j++) {
            int bands = Math.min(Psy.P_BANDS,
                    Math.min(in[is].data[j].length, in[is + 1].data[j].length));
            for (i = 0; i < bands; i++)
